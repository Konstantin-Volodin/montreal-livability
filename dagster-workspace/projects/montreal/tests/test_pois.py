...
def test_tags_by_key_merges_categories_sharing_a_key():
    # school and health both live under "amenity"; grocery under "shop".
...
def test_overpass_query_is_well_formed():
    ...
    # one selector per tag key, each carrying the Overpass bbox order (S,W,N,E).
...
def test_elements_to_points_filters_and_dedupes():
    elements = [
        ...
        {"type": "node", "id": 1, "lon": -73.5, "lat": 45.5, "tags": {"shop": "supermarket", "name": "A"}},  # dup (type, id)
        {"type": "node", "id": 2, "lon": -73.6, "lat": 45.6, "tags": {"amenity": "bench"}},  # unclassifiable
        {"type": "way", "id": 3, "tags": {"amenity": "school"}},  # no location
    ...
    assert sorted(gdf["osm_id"]) == [1, 4]  # dup, unclassifiable, and locationless dropped
...
def test_silver_categories_derive_from_bronze_tag_map():
    # the silver value->category source must stay in lockstep with the bronze tag map.
