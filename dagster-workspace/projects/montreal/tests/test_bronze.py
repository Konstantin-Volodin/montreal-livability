...
def test_contract_checks_catch_duplicates_and_nulls():
    ...
    assert results["schema_contract"] is True
    assert results["row_uniqueness"] is False
    assert results["field_completeness"] is False
...
def test_each_bronze_module_has_a_coherent_contract(module):
    ...
    assert contract.schema.get("geometry") == "geometry"
