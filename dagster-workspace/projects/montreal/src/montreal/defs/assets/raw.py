...
@dg.asset(group_name="raw_data", metadata=_BRONZE_META.append({"source": "donnees.montreal.ca", }))
...
@dg.asset(group_name="raw_data", metadata=_BRONZE_META.append({"source": "geofabrik.de", }))
def quebec_osm_pois(context: dg.AssetExecutionContext, s3_datastore: s3_datastore) -> dg.MaterializeResult:
    ...
    context.log.info(f"Combined {len(gdf)} Montreal POIs from Geofabrik Quebec extract.")
...
@dg.asset(group_name="raw_data", metadata=_BRONZE_META.append({"source": "stm.info", }))
