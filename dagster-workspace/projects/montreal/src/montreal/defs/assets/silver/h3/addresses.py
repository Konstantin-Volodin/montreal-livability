...
def _reconcile_r6_partitions(context: dg.AssetExecutionContext, desired: set[str]) -> None:
    """Reconcile dynamic r6 partitions to match desired set."""
    ...
    added, removed = len(desired - existing), len(existing - desired)
    context.log.info(f"r6 partitions: {len(desired)} cells (+{added} / -{removed})")
...
def h3_montreal_addresses(context: dg.AssetExecutionContext, s3_datastore: s3_datastore) -> dg.MaterializeResult:
    """H3-index addresses, shard by r6, and reconcile partitions."""
